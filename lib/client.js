...
p.action = function (action, callback) {
  // A string is taken to be a path for loading a page.
  ...
  action.type = action.type || "page";

  // If no sockets are set, then at least default to an empty array.
  if (action.type === "page") {
    action.sockets = action.sockets || [];
  }
...
p._http = function (action, callback) {
  ...
  request(settings, function (error, response, body) {
    ...
    if (!error && response.statusCode !== 200) {
      self.log.error("Status code " + response.statusCode + " for " + settings.url);
    }
    callback(error, self.page);
  });
};
...
p._getStatic = function (action, callback) {
  ...
  if (!action.extensions || !action.extensions.length) {
...
p._socket = function (action, callback) {
  ...
  this.log.info("Connecting socket: " + action.namespace);

  var self = this;
  var socketUrl = this.getUrl(action.namespace);
  // Is this the first socket connection for this page?
  var first = (Object.keys(this.page.sockets).length === 0);
  ...
  namespace.socket.on("error", function (error) {
    self.log.error(error);
  });
  self.page.sockets[action.namespace] = namespace;
  namespace.socket.connect();

  // Wait for connection to complete before continuing.
 ...
 * {
 *   type: "emit",
 *   // This can be omitted and defaults to "/".
 *   namespace: "/namespace"
...
p._emit = function (action, callback) {
  ...
  this.log.info("Emit for namespace: " + action.namespace + " with event: " + action.eventType);
...
p._awaitEmit = function (action, callback) {
  ...
  timeoutId = setTimeout(function () {
    ...
    callback(new Error("Timed out waiting on event " + action.eventType + " from socket " + action.namespace));
  }, action.timeout);
...
p.clear = function () {
  ...
  this.ajax = undefined;
  // Data for the last Socket.IO event sent from the server.

 ...
 *     protocol: "http"
 *   },
 *   // Optional settings for socket connections.
 *   sockets: {
 *     // If not specified in an action, this is the namespace used.
 *     defaultNamespace: ""
 *     // If not specified in an action to connect or wait on an event, this
 *     // value is used. In milliseconds. It can be a function.
 *     defaultTimeout: 500
 *   }
 * }
 *
...
function Client (config) {
  ...
  this.config.sockets = this.config.sockets || {};
  this.config.sockets.defaultNamespace = this.config.sockets.defaultNamespace || "";
  this.config.sockets.defaultTimeout = this.config.sockets.defaultTimeout || 500;
 ...
 *   Of the form function (error).
 * @return {string}
 *   The timeoutId used to wait for connection timeout. Useful for building
 *   compound connect-and-something-else functions on top of this.
 */
p._connect = function (action, callback) {
  if (!this.page) {
    callback(new Error("Action: connect: a page must be loaded before connecting with Socket.IO."));
    return;
  }

  this.setSocketActionDefaults(action);
 ...
 *   // The event type.
 *   eventType: "someEvent",
 *   // Milliseconds to wait before timing out, defaults to
 *   // config.sockets.defaultTimeout and can be a function.
 *   timeout: 2000
 * }
 *
 * @param {object} action
 ...
 * @return {string}
 *   The timeoutId used to wait for emit timeout. Useful for building
 *   compound await-and-something-else functions on top of this.
 */
p._awaitEmit = function (action, callback) {
  if (!this.page) {
    callback(new Error("Cannot await an emitted event unless a page is loaded."));
    return;
  }
...
/**
 * Wait for a period to see whether a matching event is emitted.
 *
 * The action definition follows this format:
 *
 * {
 ...
 *   // The event type.
 *   eventType: "someEvent",
 *   // Milliseconds to wait, defaults to config.sockets.defaultTimeout and can
 *   // be a function.
 *   timeout: 2000,
 *   // Can be omitted. The function will be given the same arguments as an
 *   // event listener, and should return a boolean. Only events that result
 *   // in a true response will be counted as matching. If omitted, all events
 *   // will batch.
 ...
 * @return {string}
 *   The timeoutId used to wait for emit timeout. Useful for building
 *   compound confirm-and-something-else functions on top of this.
 */
p._confirmNoMatchingEmit = function (action, callback) {
  if (!this.page) {
    callback(new Error("Cannot confirm no emitted event unless a page is loaded."));
    return;
  }

  this.setSocketActionDefaults(action);
  if (typeof action.match !== "function") {
  ...
  // Listener function to add to the socket connection.
  var listener = function () {
    // Check for the match first. We only finish up and count this a failed
    // test if the event passes the match function.
    if (!action.match.apply(this, arguments)) {
      return;
    }
    ...
    var error = new Error("Event " + action.eventType + " from socket namespace " + action.namespace + " occurred.");
    callback(error, self.socketEvent);
  };
  ...
  namespace.on(action.eventType, listener);

  return timeoutId;
};

/**
 * Wait for a period to see whether any event of the given type is emitted.
 *
 * The action definition follows this format:
 *
 * {
 ...
 *   compound confirm-and-something-else functions on top of this.
 */
p._confirmNoEmit = function (action, callback) {
  delete action.match;
  return this._confirmNoMatchingEmit(action, callback);
};

/**
 * A combination action for situations in which the server emits as soon as
 * the client connects.
 *
 * The action definition follows this format:
 *
 ...
 *   // The event type.
 *   eventType: "someEvent",
 *   // Milliseconds to wait before timing out the initial connection, defaults
 *   // to config.sockets.defaultTimeout.
 *   connectTimeout: 2000,
 *   // Milliseconds to wait before timing out waiting for the event, defaults
 *   // to config.sockets.defaultTimeout.
 *   awaitEmitTimeout: 2000,
 *   // Optional client socket configuration.
 *   socketConfig: {}
 * }
 *
 ...
 *   Of the form function (error, eventData).
 */
p._connectAndAwaitEmit = function (action, callback) {
  var connectTimeoutId, awaitTimeoutId;
  var connectAction = clone(action);
  connectAction.type = "connect";
  connectAction.timeout = action.connectTimeout;
  connectTimeoutId = this._connect(connectAction, function (error) {
    if (error) {
      // This will stop the callback firing for the awaitEmit call.
      clearTimeout(awaitTimeoutId);
      callback(error);
    }
  });
  // The synchronous part of this._connect() gives us the necessary
  // namespace instance to work with for awaiting a connection.
  var awaitEmitAction = clone(action);
  awaitEmitAction.type = "awaitEmit";
  awaitEmitAction.timeout = action.awaitEmitTimeout;
  awaitTimeoutId = this._awaitEmit(action, callback);
};

/**
 * A combination action to confirm that the server does not emit as soon as
 * a client connects.
 *
 * The action definition follows this format:
 *
 ...
 *   Of the form function (error, eventData).
 */
p._connectAndConfirmNoMatchingEmit = function (action, callback) {
  var connectTimeoutId, confirmNoEmitTimeoutId;
  var connectAction = clone(action);
  connectAction.type = "connect";
  connectAction.timeout = action.connectTimeout;
  connectTimeoutId = this._connect(connectAction, function (error) {
    if (error) {
      // This will stop the callback firing for the awaitEmit call.
      clearTimeout(confirmNoEmitTimeoutId);
      callback(error);
    }
  });
  // The synchronous part of this._connect() gives us the necessary
  // namespace instance to work with for awaiting a connection.
  var confirmNoEmitAction = clone(action);
  confirmNoEmitAction.type = "confirmNoMatchingEmit";
  confirmNoEmitAction.timeout = action.confirmNoEmitTimeout;
  confirmNoEmitTimeoutId = this._confirmNoMatchingEmit(action, callback);
};
 ...
 *   Of the form function (error, eventData).
 */
p._connectAndConfirmNoEmit = function (action, callback) {
  delete action.match;
  this._connectAndConfirmNoMatchingEmit(action, callback);
};
...
/**
 * Fill in defaults for this socket-related action.
 *
 * @param {object} action
 *   The action definition.
 * @return {object}
 *   The action definition.
 */
p.setSocketActionDefaults = function (action) {
  action.namespace = action.namespace || this.config.sockets.defaultNamespace;
  action.timeout = action.timeout || this.config.sockets.defaultTimeout;
  if (typeof action.timeout === "function") {
